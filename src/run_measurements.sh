...
for NAME in ${NAMES[@]}; do
    ...
    if [ "$NAME" == "mandelbrot_seq" ] ; then 
        ...
        while [ $NTHREADS -le 32 ]; do
            for ((i=1; i<=$ITERATIONS; i++)); do
                    perf stat -r $MEASUREMENTS ./$NAME -2.5 1.5 -2.0 2.0 $NTHREADS $SIZE >> full_$NTHREADS.log 2>&1
                    perf stat -r $MEASUREMENTS ./$NAME -0.8 -0.7 0.05 0.15 $NTHREADS $SIZE >> seahorse_$NTHREADS.log 2>&1
                    perf stat -r $MEASUREMENTS ./$NAME 0.175 0.375 -0.1 0.1 $NTHREADS $SIZE >> elephant_$NTHREADS.log 2>&1
                    perf stat -r $MEASUREMENTS ./$NAME -0.188 -0.012 0.554 0.754 $NTHREADS $SIZE >> triple_spiral_$NTHREADS.log 2>&1
                    SIZE=$(($SIZE * 2))
            done
            NTHREADS=$(($NTHREADS * 2))
            SIZE=$INITIAL_SIZE
